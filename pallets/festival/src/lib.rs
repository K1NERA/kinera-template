//** About **//
	// Festivals are one time events in which a user creates a selection of movies.
    // Users may then vote with tokens on any entries they desire before the time limit is up.
    // The movie(s) with the highest number of locked tokens wins, and each user that voted for it
    // earns rewards proportional to the weight of their vote on the winning movie(s).
    // The vote weight begins to decrease towards the end of the festival.
    // The festival creator is rewarded a percentage of the total pool.
